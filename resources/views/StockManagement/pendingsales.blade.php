...
@section('pageHead')
    <div id="page-head">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="#">Sales awaiting payment</a></li>
        </ol>
    </div>
@endsection
@section('content')
                        ...
                        <div class="row">
                            <div class="col-sm-5">
                                <div class="form-group">
                                    <label class="control-label">Invoice/Ref No</label>
                                    <select class="select_picker_customer form-control" id="ref" data-live-search="true" name="ref" onchange="Reload();">
                                        <option value="">--Select--</option>
                                        @foreach($Reflist as $list)
                                        <option value="{{ $list->refno }}" {{ (old('ref') == $list->refno ||($ref) == $list->refno  ) ? 'selected':'' }}>{{ $list->refno }}({{ $list->invoice_no }})</option>
                                        @endforeach
                                   </select>
                                </div>
                            </div>
